...
void loop() {
  ...
  if (mode == 0 & direction == 0) {
    ...
    double h = z;
    double vh = zd;
    int current_z = constrain((int)(h / 0.3), 1, areaSize[2]);
    ...
    if (current_z == hdes) {
      ...
      if ((time_now - arrival_time) >= 2.5 && arrival_time > 0) {
        episode++;
...
void updateBelief(double prex, double prey, double currentx, double currenty, int z_level) {
  ...
  int w_obs[2] = {0, 0};
  if (fabs(vx) >= 0.05) w_obs[0] = (vx > 0) ? 1 : -1;
  if (fabs(vy) >= 0.05) w_obs[1] = (vy > 0) ? 1 : -1;
